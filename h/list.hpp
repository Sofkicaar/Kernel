...
private:
    struct Elem
    {
       ...
       void* operator new(size_t size) {return MemoryAllocator::malloc(size);}
        void operator delete(void* addr) {MemoryAllocator::free(addr);}
    };
...
public:
    ...
    int size() {
        int i=0;
        for(Elem* pomoc = head; pomoc; pomoc=pomoc->next) {
            i++;
        }
        return i;
    }
